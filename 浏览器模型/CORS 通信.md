...
- [CORS 通信](#cors-通信)
    ...
    - [简单请求](#简单请求)
        - [1. 基本流程](#1-基本流程)
            - [（1）Access-Control-Allow-Origin](#1access-control-allow-origin)
            - [（2）Access-Control-Allow-Credentials](#2access-control-allow-credentials)
            - [（3）Access-Control-Expose-Headers](#3access-control-expose-headers)
        - [2. withCredentials 属性](#2-withcredentials-属性)
    - [非简单请求](#非简单请求)
        - [1. 预检请求](#1-预检请求)
            - [（1）Access-Control-Request-Method](#1access-control-request-method)
            - [（2）Access-Control-Request-Headers](#2access-control-request-headers)
        - [2. 预检请求的回应](#2-预检请求的回应)
            - [（1）Access-Control-Allow-Methods](#1access-control-allow-methods)
            - [（2）Access-Control-Allow-Headers](#2access-control-allow-headers)
            - [（3）Access-Control-Allow-Credentials](#3access-control-allow-credentials)
            - [（4）Access-Control-Max-Age](#4access-control-max-age)
        - [3. 浏览器的正常请求和回应](#3-浏览器的正常请求和回应)
...
上面的 HTTP 回应中，关键的是Access-Control-Allow-Origin字段，表示`http://api.bob.com`可以请求数据。该字段也可以设为星号，表示同意任意跨源请求。
